...
void TestPhysicsProcess() {
  Timer timer; // 记时器
  TFile* f = TFile::Open("/Users/yemingxin/nptool/Outputs/Simulation/EspriPhysics3.root", "READ");
  ...
  std::map<int, std::pair<double, double> > electronEnergyTheta;
  int electronIndex = 0;
  ...
  for (Long64_t i = 0; i < nentries; i++) 
  {
    ...
    for (int j = 0; j < multiplicity; j++) 
    {
      ...
      if (particleName == "e-") 
      {
        ...
        if (electronIndex <= 20) 
        {
          electronEnergyTheta[electronIndex] = std::make_pair(ti->GetKineticEnergy(j), ti->GetTheta(j));
        }
      }
    }
 ...
 std::cout << std::endl;
 std::cout << "First 20 emitted electron energy-theta:" << std::endl;
 for (const auto& item : electronEnergyTheta) 
 {
  std::cout << "electron " << item.first << " : " << item.second.first << " MeV"
            << " , " << item.second.second << " deg" << std::endl;
 }
