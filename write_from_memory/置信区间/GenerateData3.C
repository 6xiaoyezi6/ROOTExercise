/*
 * 功能：在 x ∈ [0, 2] 上生成 21 个 Poisson 计数点，比较二次多项式加权
 *       最小二乘与 Poisson 最大似然拟合及其 95% 逐点置信带，输出
 *       GenerateData3.root 和 GenerateData3.pdf。
 * 方法：代码开头的缩放系数 K 控制整体计数强度，默认 K=5；x 按 0.1
 *       等距取点并包含左右端点，N_i~Poisson(mu_i)，mu=K*(x^2+x)；
 *       chi2 误差取 sqrt(max(N,1))，RooFit 使用条件 Poisson 似然；
 *       两种置信带由参数协方差矩阵线性传播，终端表格比较真实、WLS 和 MLE 系数。
 * 注意事项：K 必须大于 0；固定随机种子保证可重现；x=0 时真实均值为 0，
 *           RooFit 将非正均值截到 1e-9 以保持 Poisson 似然有效；置信带不是
 *           预测带或同时置信带；低计数下 sqrt(max(N,1)) 只是 Gaussian 近似。
 */
...
void GenerateData3() {
  ...
  constexpr Double_t trueA0 = 0.0, trueA1 = 1.0, trueA2 = 1.0;
  ...
  g1->SetName("g1"); g1->SetTitle("Poisson quadratic data;x;Counts");
  g1->SetMarkerStyle(20); g1->SetMarkerSize(0.9);
  g1->SetMarkerColor(kBlack); g1->SetLineColor(kBlack);
  TF1 *fTrue = new TF1("fTrue", "pol2", xMin, xMax);
  fTrue->SetParameters(K * trueA0, K * trueA1, K * trueA2);
  fTrue->SetLineColorAlpha(kRed, 0.45); fTrue->SetLineWidth(3);

  TF1 *fChi2 = new TF1("fChi2", "pol2", xMin, xMax);
  fChi2->SetParameters(K * trueA0, K * trueA1, K * trueA2);
  fChi2->SetParNames("a0", "a1", "a2");
  ...
  RooRealVar rooA2("rooA2", "a2", fChi2->GetParameter(2),
                   -coefficientLimit, coefficientLimit);
  RooArgList meanArgs(rooA0, rooA1, rooA2, rooX);
  RooFormulaVar rooMean("rooMean", "max(1e-9, @0 + @1*@3 + @2*@3*@3)", meanArgs);
  ...
  TF1 *fMLE = new TF1("fMLE", "max(1e-9, [0]+[1]*x+[2]*x*x)", xMin, xMax);
  fMLE->SetParameters(rooA0.getVal(), rooA1.getVal(), rooA2.getVal());
  fMLE->SetParNames("a0", "a1", "a2");
  fMLE->SetLineColor(kGreen + 2); fMLE->SetLineStyle(kDashed); fMLE->SetLineWidth(3);
  RooArgList mleParameters(rooA0, rooA1, rooA2);
  ...
  auto FillConfidenceBand = [&](TGraphErrors *band, TF1 *fitFunction,
    ...
    for (Int_t i = 0; i < confidencePointCount; ++i) {
      ...
      const Double_t basis[3] = {1.0, xx, xx * xx};
      Double_t rawMu = 0.0;
      for (Int_t a = 0; a < 3; ++a) rawMu += fitFunction->GetParameter(a) * basis[a];
      const Bool_t meanIsClipped = protectPoissonMean && rawMu <= 1e-9;
      const Double_t mu = meanIsClipped ? 1e-9 : rawMu;
      Double_t gradient[3] = {0.0}, variance = 0.0;
      for (Int_t a = 0; a < 3; ++a)
  ...
  c1->SaveAs("GenerateData3.pdf");

  TFile *outputFile = new TFile("GenerateData3.root", "RECREATE");
