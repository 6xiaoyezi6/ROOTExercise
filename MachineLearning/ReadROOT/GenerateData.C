/*
功能：在 x=[-2,2]、步长 0.2 的网格上生成二次函数含噪数据。
模型：y = x^2 + x + 2 + noise，其中 noise ~ N(0,sigma^2)，sigma=1。
输出：Data.root 保存 x、y、noise；Data.pdf 绘制数据、1 sigma 误差棒和理论曲线。
*/
...
void GenerateData() {
  ...
  const Double_t xStep = 0.2;
  const Double_t sigma = 1.0;
  const Int_t pointCount = 21;
  ...
  g1->SetMaximum(15.0);
  g1->Draw("AP");
  g1->GetXaxis()->SetLimits(-2.0, 2.0);
  g1->GetXaxis()->CenterTitle(kTRUE);
  g1->GetYaxis()->CenterTitle(kTRUE);

  TF1 *f1 = new TF1("f1", "x*x + x + 2", -2.0, 2.0);
